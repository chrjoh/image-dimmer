...
/// Read image data from the given file create a new image with the specified overlay
/// and save the image to disk
/// The overlay is constucted to got from the botom to 60% of the image hight where it will be no
/// overlay and up to the top increasing the overlay color
...
mod tests {
    ...
    #[test]
    fn test_generate_from_url_with_mock() {
        ...
        // Prepare output file
        let output_file = Builder::new().suffix(".png").tempfile().unwrap();
        let out_path = output_file.path().to_owned();
        drop(output_file); // allow the function to write to the path
        ...
        assert!(Path::new(&out_path).exists(), "Output file was not created");
    ...
    #[test]
    fn test_generate_creates_output_file() {
        let input_file = create_temp_image_file(Rgba([50, 100, 150, 255]), 4, 4);
        let output_file = tempfile::Builder::new().suffix(".png").tempfile().unwrap();
    ...
    fn create_temp_image_file(color: Rgba<u8>, width: u32, height: u32) -> NamedTempFile {
        let img = dummy_image(width, height, color);
